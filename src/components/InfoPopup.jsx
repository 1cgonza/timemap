...
export default ({ ui, app, methods }) => {
  function renderIntro() {
    let introCopy = copy[app.language].legend.default.intro;
  ...
  function renderHalfWithDot() {
    ...
    return [
      <style key="style">{`.svg-demo { max-width: 30px } .first { fill: ${firstFill} } .second { fill: ${secondFill} } .demo-text { font-size: 9pt; color: white; font-weight:900 }`}</style>,
      <svg key="svg" viewBox="0 0 30 30" className="svg-demo">
        <g className="location demo-element" transform="translate(15,15)">
          <path
            className="location-event-marker first"
            id="arc_0"
            d="M 10 0 A 10 10 0 0 1 -10 1.2246467991473533e-15 L 0 0  L 10 0 Z"
          />
          <path
            className="location-event-marker second"
            id="arc_1"
            d="M -10 1.2246467991473533e-15 A 10 10 0 0 1 10 -2.4492935982947065e-15 L 0 0  L -10 1.2246467991473533e-15 Z"
          />
          <text className="location-count demo-text" dx="-4" dy="4">
            2
          </text>
        </g>
      </svg>,
  ...
  function renderCategoryColors() {
    ...
    return categories.map((category, i) => (
      <div key={`cat-${i}`} className="legend-section">
    ...
    return (
      <div className={`infopopup ${app.flags.isInfopopup ? '' : 'hidden'}`}>
        <div className="legend-header">
          <button onClick={methods.onClose} className="side-menu-burg over-white is-active">
            <span />
          </button>
          <h2>{copy[app.language].legend.default.header}</h2>
        </div>
        {renderIntro()}
        <div>{renderCategoryColors()}</div>
        <br />
        <div className="legend">
          <div className="legend-container">
            <div className="legend-item one">{renderHalfWithDot()}</div>
            <div className="legend-item three">{copy[app.language].legend.default.notation}</div>
          </div>
        </div>
        <br />
        <div>
        ...
        {ui.style.arrows
          ? Object.keys(ui.style.arrows).map((arrowName, i) => (
              <div key={`arr${i}`} className="legend-section">
  ...
  return <div>{renderView2DLegend()}</div>;
};
