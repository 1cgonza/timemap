...
class Map extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (hash(bounds) !== hash(this.props.app.map.bounds) && bounds !== null) {
  ...
  initializeMap() {
    ...
    if (
      ...
      firstLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png');
    }
    ...
    map.on('zoomstart', () => {
      if (this.svgRef.current !== null) this.svgRef.current.classList.add('hide');
    });
    map.on('zoomend', () => {
      if (this.svgRef.current !== null) this.svgRef.current.classList.remove('hide');
    });
  ...
  alignLayers = () => {
    ...
    const transform = window.getComputedStyle(mapNode).getPropertyValue('transform');
  ...
  renderSites() {
    return (
      <Sites sites={this.props.domain.sites} projectPoint={this.projectPoint} isEnabled={this.props.app.views.sites} />
    );
  }
...
function mapStateToProps(state) {
  return {
    domain: {
      ...
      cais: state.domain.cais ? state.domain.cais : [],
      victimas: state.domain.victimas ? state.domain.victimas : [],
    },
    app: {
