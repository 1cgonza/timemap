  ...
  renderFeature() {
    ...
    return (
      <div>
          ...
          {translations &&
            translations.map((trans, idx) => {
              ...
              if (featureLang !== langIdx) {
                return (
                  <div onClick={() => this.setState({ featureLang: langIdx })} className="trans-button">
                    {trans.code}
                  </div>
                );
              } else {
                return (
                  <div onClick={() => this.setState({ featureLang: 0 })} className="trans-button">
                    ENG
                  </div>
                );
              }
        ...
        <Player className="source-video" poster={source.poster} playsInline src={source.file} />
      </div>
    );
  }
    ...
    if (!this.props.cover) {
      return (
        <div className="default-cover-container">
          You haven't specified any cover props. Put them in the values that overwrite the store in{' '}
          <code>app.cover</code>
        </div>
      );
    }
        ...
        <div className="cover-content">
          {this.props.cover.bgVideo ? (
            <div className={`fullscreen-bg ${!this.props.showing ? 'hidden' : ''}`}>
              <video loop muted autoPlay preload="auto" className="fullscreen-bg__video">
          ...
          {this.props.cover.subtitle ? <h3 style={{ marginTop: 0 }}>{this.props.cover.subtitle}</h3> : null}
          {this.props.cover.subsubtitle ? <h5>{this.props.cover.subsubtitle}</h5> : null}
