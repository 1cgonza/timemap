...
function childrenToToggle(filter, activeFilters, parentOn) {
  ...
  const childKeys = Object.entries(children).flatMap((filter) => childrenToToggle(filter, activeFilters, isOn));
...
function FilterListPanel({ filters, activeFilters, onSelectFilter, language }) {
  function createNodeComponent(filter, depth) {
    ...
    return (
      <li key={key.replace(/ /g, '_')} className={'filter-filter'} style={{ marginLeft: `${depth * 20}px` }}>
        <Checkbox
          label={key}
  ...
  function renderTree(filters) {
    ...
    return <div>{Object.entries(aggregatedFilterPaths).map((filter) => createNodeComponent(filter, 1))}</div>;
  }
